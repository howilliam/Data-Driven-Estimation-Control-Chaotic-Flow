...
% two paths created 1 to access the mat files from extracted data and other
% path to save the POD into a different directory location

%path='\\icnas1.cc.ic.ac.uk\jaa21\Desktop\MSCproj\HPC data\DNS DATA (mat files)\';
%path='/home/wh219/FYP/Open Loop/'; %% path to saved grid data open loop
path='/home/wh219/FYP/Controller/Controller Save/'; %% path to saved grid data
%pathPOD='/home/wh219/FYP/Open Loop/'; %path to save POD open loop
pathPOD='/home/wh219/FYP/Controller/Controller Save/'; %path to save POD exciatation / control
load([path 'Griddata_Control_9000_03_05']); % load griddata that you got from data extraction

%% DATASET size

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% U = U(1:end-1,:);
% V = V(1:end-1,:);                      %% all these modified to get rid of NaN values
% x_plotting = x_plotting(1:end-1);
% y_plotting = y_plotting(1:end-1);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
T=10010*dt:dt*r:100000*dt;% flow time
n=length(x_plotting);
...
grid on; xlim([1 limit]); xticks([0:5:limit]); ylim([0 0.16]);
yBox = [0, 0, 0.5, 0.5, 0];
...
% yBox = [0, 0, 100, 100, 0];
% xBox = [5, 10, 10, 5, 5];
xBox = [50, 100, 100, 50, 50];
retained2= patch(xBox, yBox, 'blue', 'FaceColor', 'blue', 'EdgeColor','none','FaceAlpha', 0.2); 
% xBox = [10, 150, 150, 10, 10];
% xBox = [50, 150, 150, 50, 50];
legend([retained, retained2],'Reduced-order model','Truncated modes','Fontsize',14,'interpreter','latex');
% patch(xBox, yBox, 'black', 'FaceColor', 'EdgeColor','none','FaceAlpha', 0.2);
...
for i = 1:16
    ...
    set(gca,'Fontsize',14); 
    %fill(s_geo,h_geo_u,'k'); hold on; fill(s_geo,h_geo_L,'k');  %% this is
    %fish data
    %hold on;fill(x_cyl,y_cyl,'k');
end 
...
for i = 1:16
    ...
    set(gca,'TickLength',[0 0]);
    set(gca,'Fontsize',14); 
    %% fill(s_geo,h_geo_u,'k'); hold on; fill(s_geo,h_geo_L,'k'); fish data
%     hold on;
%     fill(x_cyl,y_cyl,'k');
end 
...
%% this line may need to change if i keep on changing the my star ccm
snapshot = find(round(T,4) == 30000*0.01); % choose snapshot to compare
...
for j = 1:length(modes)
    ...
    set(gca,'TickLength',[0 0]);
    set(gca,'Fontsize',12); 
    % fill(s_geo,h_geo_u,'k'); hold on; fill(s_geo,h_geo_L,'k');
    %hold on;fill(x_cyl,y_cyl,'k');
...
save([pathPOD 'POD_Control_9000_03_06'],'a','phi','Y_mean','Y','lambda')