%% Script for System Identification and Kalman filter
...
% m_H=1:input('how many modes to estimate/construct H? (no: of sensor points)'); %% this will determine the number of sensor points
...
if sysID_operation == 0 % Not computed
        ...
        if input('save system identification?')==1
            save([pathSID 'SYSID_Nx' num2str(Nx) '_M_' num2str( m(end)) '.mat' ],'sysID','x0', '-v7.3')
        end 
...
sensor_point = [ceil(1:1:500)]; % OR IF A SET OF SENSORS IS REQUIRED USE
%VECTOR FORM []
